...
async function getSaveFile(id: string) {
  return await readFile(path.join(savesDir, id, 'SaveGameInfo'), { encoding: 'utf-8' })
}

contextBridge.exposeInMainWorld('backend', {
  async chooseFolder(defaultPath?: string) {
    return await dialog.showOpenDialog({ properties: ['openDirectory'], defaultPath }).then(value => value.filePaths[0] ?? null);
  },
  ...
  async getSaveFiles() {
    ...
    )).then(saves => saves.filter((save): save is { id: string, data: string; } => save.data !== null));
  },

  async watchSaveFile(id: string) {
    const savePath = path.join(savesDir, id, 'SaveGameInfo');

    return readable<string>(await readFile(savePath, { encoding: 'utf-8' }), set => {
