 ...
 * Run as `deno run --allow-read --allow-net --allow-write import.ts <path to stardew valley installation>/Content > /path/to/game-info.json`
 */
...
function getSprite(id: string): string {
  ...
  return base64.encode(png.encode(image.data, image.width, image.height));
}
//#endregion Sprites
